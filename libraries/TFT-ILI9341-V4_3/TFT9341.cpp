...
void tft9341::clrscr ()
{
	...
	this->x = 0;
	this->y = 0;
	sector(0,this->width-1,0,this->height-1);//320x240
	for (i=0;i<this->height;i++)
		for (j=0;j<this->width;j++)
	writeData2bytes(this->background>>8,this->background);
}
...
void tft9341::putChar(char c)
{
	...
	if ((this->x >= this->width) || (this->y >= this->height) || ((this->x + 6 * this->fontSize - 1) < 0) || ((this->y + 8 * this->fontSize - 1) < 0))
		return;
	...
	{		
		...
		for (j = 0; j<8; j++)
		{
			if (line & 0x1)
			{
				if (this->fontSize == 1) // default size
					drawPixel(this->x+i, this->y+j);
				else
					fillRect(this->x+(i*this->fontSize), this->y+(j*this->fontSize), this->fontSize, this->fontSize);
			} 
			else if (this->background != this->color)
			{
				switchColors();
				if (this->fontSize == 1) // default size
					drawPixel(this->x+i, this->y+j);
...
{
	sector(x,x,y,y);
	writeData2bytes(this->color>>8,this->color);	
}
...
uint16_t touch::getX()
{
	...
	uint16_t dx;
	#ifdef PCF
	exPort.digitalWrite (PCFDEVICE,PCFPORT,0);
	#else
	T_SS = 0;
	#endif
	SPI.transfer(0x90);
	...
	#ifdef PCF
	exPort.digitalWrite (PCFDEVICE,PCFPORT,1);
	#else
	T_SS = 1;
	#endif
	return (dx);
...
void touch::readxy()
{
	...
	this->y=getY();
	if (rotation==0){
	// this->x=((this->x-270)/10.5);
	//this->y=((this->y-300)/14);
	
	this->x=(320-(this->x-270)/10.5);
	this->y=(240-(this->y-300)/14);
	}
	else if (rotation==2){
//	this->x=320-((this->x-270)/10.5);
//	this->y=240-((this->y-300)/14);
